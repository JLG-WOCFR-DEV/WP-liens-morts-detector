...
if (!class_exists('RemoteRequestClient')) {
...
if (!class_exists('LinkScanController')) {
    class LinkScanController {
        ...
        public function run($batch = 0, $is_full_scan = false, $bypass_rest_window = false) {
            return $this->queue->run($batch, $is_full_scan, $bypass_rest_window);
        }
    }
}
...
function blc_make_scan_queue(RemoteRequestClient $client) {
    return new ScanQueue($client);
}

function blc_make_link_scan_controller(ScanQueue $queue) {
    return new LinkScanController($queue);
}
...
function blc_perform_check($batch = 0, $is_full_scan = false, $bypass_rest_window = false) {
    ...
    return $controller->run($batch, $is_full_scan, $bypass_rest_window);
}
